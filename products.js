...
function loadProducts() {
    ...
    try {
        const unsubscribe = onSnapshot(collection(db, 'products'), (querySnapshot) => {
            ...
            grid.innerHTML = `<div class="p-3 text-center text-danger">Error loading products: ${error.message}</div>`;
        });
    } catch (error) {
        console.error('Error setting up products listener:', error);
        grid.innerHTML = `<div class="p-3 text-center text-danger">Error connecting to database: ${error.message}</div>`;
    }
}
...
window.selectProduct = function(productId) {
    ...
    const clickedItem = document.querySelector(`[onclick="selectProduct('${productId}')"]`);
    if (clickedItem) clickedItem.classList.add('active');
    
    const finalPrice = calculateFinalPrice(product.price, product.discount, product.discountType);
    
    if (window.innerWidth <= 768) {
        // Mobile: Show modal like orders.html
        ...
        <div class="product-detail-view">
            <div class="detail-header">
                ...
                <div class="header-actions">
                    <button onclick="editProduct('${product.id}')" class="btn btn-outline-primary btn-sm me-2">
                        <i class="fas fa-edit me-1"></i>Edit
                    </button>
                    <button onclick="addStock('${product.id}')" class="btn btn-outline-success btn-sm me-2">
            ...
            <div class="detail-content">
                <div class="product-images">
                    ${product.images && product.images.length > 0 ? 
                        product.images.map(img => `<img src="${img}" alt="Product image" style="height: 200px; object-fit: cover; margin: 5px;">`).join('') : 
...
// Show mobile product modal (like orders.html)
function showMobileProductModal(product) {
    const finalPrice = calculateFinalPrice(product.price, product.discount, product.discountType);
    
    // Create modal if it doesn't exist
    let modal = document.getElementById('mobile-product-modal');
    if (!modal) {
        ...
        modal.innerHTML = `
            <div class="mobile-modal-header">
                <button class="mobile-back-btn" onclick="closeMobileProductModal()"><i class="fas fa-arrow-left"></i></button>
                <h3 id="mobile-product-title">Product Details</h3>
            </div>
            <div class="mobile-modal-content" id="mobile-product-content">
                <!-- Mobile product details loaded here -->
            </div>
            <div class="mobile-modal-footer">
                <button class="btn btn-outline-primary btn-sm me-2" onclick="editProduct(window.selectedProductId)">
                    <i class="fas fa-edit me-1"></i>Edit
                </button>
                <button class="btn btn-outline-success btn-sm me-2" onclick="addStock(window.selectedProductId)">
                    <i class="fas fa-plus me-1"></i>Stock
                </button>
                <button class="btn btn-outline-danger btn-sm" onclick="deleteProduct(window.selectedProductId)">
                    <i class="fas fa-trash me-1"></i>Delete
                </button>
            </div>
        `;
    ...
    // Store selected product ID globally
    ...
    document.getElementById('mobile-product-content').innerHTML = `
        <div class="form-section">
            ...
            <div class="product-images">
                ${product.images && product.images.length > 0 ? 
                    product.images.map(img => `<img src="${img}" alt="Product image" style="max-width: 100%; height: 200px; object-fit: cover; border-radius: 8px; margin: 5px;">`).join('') : 
                    '<img src="https://via.placeholder.com/300x200" alt="No image" style="max-width: 100%; height: 200px; object-fit: cover; border-radius: 8px;">'}
                ${(product.stock || 0) === 0 ? `<div class="images-area-overlay">OUT OF STOCK</div>` : ''}
            </div>
        </div>
        <div class="form-section">
...
window.editProduct = (productId) => {
    ...
    if (window.innerWidth <= 768) {
        // Mobile: Show Bootstrap modal
        let modal = document.getElementById('mobile-edit-modal');
        if (!modal) {
            modal = document.createElement('div');
            modal.id = 'mobile-edit-modal';
                ...
                <div class="modal-dialog modal-fullscreen">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form id="mobile-edit-form" onsubmit="updateProduct(event, '${productId}')">
                                <div class="mb-3">
                                ...
                                <div class="row mb-3">
                                    <div class="col-6">
                                        ...
                                        <input type="number" class="form-control" id="edit-price" step="0.01" value="${product.price}" required>
                                    </div>
                                    <div class="col-6">
                                        <label class="form-label">Stock</label>
                                        <input type="number" class="form-control" id="edit-stock" value="${product.stock || 0}">
                                    </div>
                                </div>
                                <div class="mb-3">
                                    <label class="form-label">Category *</label>
                                    <select class="form-select" id="edit-category" required>
                                        <option value="${product.category || ''}">${product.category || 'Select Category'}</option>
                                    </select>
                                </div>
                                <div class="mb-3">
                        ...
                        <div class="modal-footer">
                            ...
                            <button type="submit" form="mobile-edit-form" class="btn btn-primary">Update Product</button>
                        </div>
                    </div>
                </div>
            `;
                    ...
                    <form class="product-form" id="desktop-edit-form" onsubmit="updateProduct(event, '${productId}')">
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-4">
                                ...
                                <input type="number" class="form-control" id="edit-discount" min="0" value="${product.discount || 0}" placeholder="0">
                            </div>
                        </div>
                        <div class="row mb-3">
                            <div class="col-md-6">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="number" class="form-control" id="edit-min-stock" value="${product.minStock || 0}" placeholder="0">
                            </div>
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Product Images *</label>
                            <button type="button" class="btn btn-primary" onclick="document.getElementById('edit-image').click()">
                                <i class="fas fa-upload me-2"></i>Upload Images
                            </button>
                            <input type="file" id="edit-image" accept="image/*" multiple onchange="previewEditImages(this)" style="display: none;">
                            <div class="image-preview mt-2" id="edit-image-preview"></div>
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Description</label>
                            <textarea class="form-control" id="edit-description" rows="3">${product.description || ''}</textarea>
                        ...
                        <div class="row mb-3">
                            <div class="col-md-3">
                                ...
                                <select class="form-select" id="edit-brand"></select>
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">Color</label>
                                <select class="form-select" id="edit-color-attr"></select>
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">Size</label>
                                <select class="form-select" id="edit-size-attr"></select>
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">Unit</label>
                                <select class="form-select" id="edit-unit-attr"></select>
                            </div>
                        </div>
                    </form>
                </div>
