// Gallery Management
import { db, storage } from '../../frontend/config/firebase-config.js';
...
function showUploadModal() {
    ...
    modal.innerHTML = `
        <div style="background: white; padding: 30px; border-radius: 8px; text-align: center; min-width: 300px;">
            <h3>Upload Image</h3>
            <br>
            <button class="btn" onclick="document.getElementById('image-upload').click()">Choose File</button>
            <br><br>
...
window.setHeroImage = function(url) {
    ...
    modal.innerHTML = `
        <div style="background: white; padding: 30px; border-radius: 8px; min-width: 300px;">
            ...
            <br>
            <button class="btn" onclick="applyHeroImage('${url}')">Apply</button>
            <button class="btn" onclick="document.getElementById('hero-image-modal').remove()" style="margin-left: 10px;">Cancel</button>
        </div>
    `;
    document.body.appendChild(modal);
};
    ...
    modal.innerHTML = `
        <div style="text-align: center; max-width: 90%; max-height: 90%; position: relative;">
            ...
            <div style="display: flex; gap: 15px; justify-content: center; margin-top: 20px; flex-wrap: wrap;">
                <button onclick="event.stopPropagation(); setHeroImage('${url}')" style="background: #3498db; color: white; border: none; padding: 10px 15px; border-radius: 5px; cursor: pointer;"><i class="fas fa-image"></i> Set Hero</button>
                <button onclick="event.stopPropagation(); copyUrl('${url}')" style="background: #2ecc71; color: white; border: none; padding: 10px 15px; border-radius: 5px; cursor: pointer;"><i class="fas fa-copy"></i> Copy URL</button>
                <a href="${url}" download="${name}" onclick="event.stopPropagation()" style="background: #9b59b6; color: white; border: none; padding: 10px 15px; border-radius: 5px; cursor: pointer; text-decoration: none; display: inline-block;"><i class="fas fa-download"></i> Download</a>
                <button onclick="event.stopPropagation(); deleteImageFromModal('${id}', '${path}', this)" style="background: #e74c3c; color: white; border: none; padding: 10px 15px; border-radius: 5px; cursor: pointer;"><i class="fas fa-trash"></i> Delete</button>
            </div>
        </div>
    `;
    document.body.appendChild(modal);
